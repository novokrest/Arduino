...
KeyboardSimulator::KeyboardSimulator(MAX3421E &max, USB &usb)
	: _max(max), _usb(usb)
{
	_numLock = false;
	_capsLock = false;
...
int KeyboardSimulator::KeyboardPoll()
{
    ...
    for (i = 2; i < 8; i++) {
        ...
        if (true || IsNewKey(buffer[i])) {   //if new key
            switch (buffer[i]) {
                case CAPSLOCK:
                    _capsLock =! _capsLock;
...
void KeyboardSimulator::Run()
{
	while (true) {
		_max.Task();
		_usb.Task();
		if (_usb.getUsbTaskState() == USB_STATE_CONFIGURING) {  //wait for addressing state
			KeyboardInit();
			_usb.setUsbTaskState(USB_STATE_RUNNING);
		}
		if (_usb.getUsbTaskState() == USB_STATE_RUNNING) {  //poll the keyboard  
			byte status;
			status = KeyboardPoll();
		}
    }
